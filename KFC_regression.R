...
#require(Rcpp)
#sourceCpp("D:/PhD-Works/Codes-PhD/official-1st-paper/KFC-GitHub/BregmanDivergences.cpp")  # set directory for "BregmanDivergences.cpp" 
...
kernelCOBRA = function(train.design,
                       ...
                       set.Kernels = setKernels(kernels = c("naive", "uniform", "epanechnikov", "biweight", "triweight", 
                                                ...
                                                sigma = 1, rho = 1),
                       print.info = TRUE, 
                       print.train.results = TRUE, angle = 10) {
  ...
  predict.test = function(x,
    ...
    pred.naive = function(eps, M) {
      ...
      selec = id == M
      val = sum(train.responses[selec]) / sum(selec)
                       ...
                       FUN = function(kern)
                         return(
                           optimizeSmoothParameter(
                             ...
                             set.Kernels = setKernels(kernels = kern, sigma = sigma, rho = rho), angle = angle
                           )
                         )
    )
  }
...
KFCreg = function(train.input,
  ...
  if(is.null(F_step$formula)){
    single.model = lm(y.train0 ~ ., data = as.data.frame(x.train0))
    pred = predict.lm(single.model, newdata = as.data.frame(x.test))
    single.mse = mean((pred - y.test) ^ 2)
  }
  else{
    single.model = lm(F_step$formula, data = as.data.frame(x.train0))
    pred = predict.lm(single.model, newdata = as.data.frame(x.test))
    single.mse = mean((pred - y.test) ^ 2)
  }
  
  # F step: Fitting the corresponding model on each observed cluster
  model = fitClusterRegressionModel(x.train1, y.train1, kms, formula = F_step$formula)
  y.hat.train0 = matrix(model$Fitted.values, nrow = length(y.train1), byrow = FALSE)
  y.hat.train = matrix(0, nrow = length(y.train2), ncol = METH)
  y.hat.test = matrix(0, n.test, METH)
  colnames(y.hat.train) = k.meth.name
  mse = c()
  ...
  for (j in 1:METH) {
    ...
    for (l in 1:K_step$K) {
      ...
      #  x.temp = t(x.temp)
      y.hat.train[test.id2, j] = x.temp2 %*% as.numeric(model$Coefficients[[j]][[l]])
      y.hat.test[test.id_test, j] = x.temp_test %*% as.numeric(model$Coefficients[[j]][[l]])
    }
  }
  all.pred.train = matrix(0, ncol = METH, nrow = n.train0)
  if(SPD){
    all.pred.train[train_id0,] = y.hat.train0
    all.pred.train[-train_id0,] = y.hat.train
  } else{
    all.pred.train = y.hat.train
  }
  pred.train = y.hat.train
  ...
  time.taken <- end.time - start.time
  if (is.null(y.test)) {
    return(list(train.predict = all.pred.train,
