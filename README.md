...
- $K$-step: $K$-means algorithm with $M$ Bregman divergences $\mathcal{B}={{B_j}}_{j=1}^M$ (see, for example, [Banerjee (2005)](chrome-extension://oemmndcbldboiebfnladdacbdfmadadm/https://www.jmlr.org/papers/volume6/banerjee05b/banerjee05b.pdf) ). Depending on the theoretical property of Bregman divergences, different Bregman divergences may cluster the input data into different structures. At the end of this step, we have $M$ different partition structures of the input data namely $ \mathcal{P}_1=(P_{1k})_{k=1}^K, ..., \mathcal{P}_M=(P_{Mk})_{k=1}^K $.
