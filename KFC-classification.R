...
fitClusterLogitModel = function(data, target, methods, formula = NULL){
  ...
  for(i in 1:m){
    ...
    for(j in 1:K){
      ...
      y.fit[id,i] = as.factor(ifelse(p.fit[id,i] >= 0.5,1,0))
...
kernelAggregateClassification = function(train.design,
  ...
  f.naive = function(y){
    ...
    res = sapply(1:M, FUN = function(t){
      y.hat = ifelse(mean(k[w == t]) >0.5, 1, 0)
...
mixCOBRAclass = function(train.input,
  ...
  if(print.info){
    ...
    if ((length(intersect(kers, c("naive", "uniform"))) > 0) & set.Kernels$full.naive)
      cat("\n\t- full-machine is used for naive/unifrom kernel\n")
    else{
      if (length(intersect(kers, c("naive", "uniform"))) > 0)
        cat("\n\t- alpha-machine is used for naive/unifrom kernel\n")
    }
    if ((L > 1) | ALL) {
      if (!ALL) {
        if (L == 2)
...
KFCclass = function(train.input,
  ...
  model = fitClusterLogitModel(x.train, y.train, kms, formula = F_step$formula)
  pred.train.prob = y.hat.train = matrix(model$fitted.prob, nrow = n.train, byrow = FALSE)
  pred.train.y = ifelse(y.hat.train > 0.5, 1, 0)
  colnames(y.hat.train) = colnames(pred.train.y) = k.meth.name
  # Single model without clustering
  if(is.null(F_step$formula)){
    single.model = glm(y.train ~ ., data = as.data.frame(x.train), family = binomial(link='logit'))
    pred = predict.glm(single.model, as.data.frame(x.test), type = "response")
    single.error = mean(as.numeric(pred > 0.5) != y.test)
  }
  else{
    single.model = glm(F_step$formula, data = as.data.frame(x.train), family = binomial(link='logit'))
    pred = predict.glm(single.model, as.data.frame(x.test), type = "response")
    single.error = mean(as.numeric(pred > 0.5) != y.test)
  }
  ...
  for (j in 1:METH) {
    ...
    for (l in 1:K_step$K) {
      ...
      if (ncol(x.temp) != length(model$coefficients[[j]][[l]]))
        x.temp = t(x.temp)
      y.hat.test[test.id, j] = sigmoid(x.temp %*% as.numeric(model$coefficients[[j]][[l]]))
    }
  }
    ...
    if (is.null(y.test)) {
      ...
      if(BOTH){
        ...
        y.hat.test = list(pred.candidate = pred.test.y,
                          ...
                          pred.mixcobra = pred.mix$prediction)
        tab = data.frame(
          mse.test = c(err, cob.err, mix.err),
          mse.single = single.error,
          row.names = name
        )
      }
      else{
...
tab = read.table(url("https://archive.ics.uci.edu/ml/machine-learning-databases/00267/data_banknote_authentication.txt"), sep = ",", header = TRUE)
df = tab[,1:4]
Y = tab[,5]
n = length(Y)
train = sample(n,0.8*n)
kfc = KFCclass(train.input = df[train,], 
               train.responses = Y[train], 
               test.input = df[-train,],
               test.responses = Y[-train], 
               K_step = Kstep(K = 3, max.iter = 500, method = c("exponential", "euclidean")),
               F_step = Fstep(single.model = TRUE),
               C_step = Cstep(method = "mixed", kernels = c("exp"), min.h = 1e-5,
                              max.h = 0.001, n.h = 50, figure = TRUE, show.info = TRUE, n.cv = 10,
                              min.alpha = 1e-100, min.beta = 1e-100, max.alpha = 0.5, max.beta = 0.1,
                              n.alpha = 30, n.beta = 10))

library(randomForest)
rf = randomForest(x = as.data.frame(tab[train,]), y = as.factor(Y[train]), ntree = 100)
pred1 = predict(rf, as.data.frame(tab[-train,]), type = "class")
library(adabag)
tab$X0 = as.factor(tab$X0)
boost <- boosting(X0~., data = as.data.frame(tab[train,]), mfinal = 100)
pred2 <- predict(boost, as.data.frame(tab[-train,]), type = "class")

err = c(kfc$table$test.error, mean(pred1 != Y[-train]), mean(pred2$class != Y[-train]))
names(err) = c(rownames(kfc$table), "randomForest", "boost")
err
